    ...
    <>
      ...
      {deleteGuard && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0d1f12] p-6 shadow-2xl">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-500/15">
                <Trash2 className="h-5 w-5 text-rose-400" />
              </div>
              <div>
                <h3 className="text-lg font-bold text-white">Delete Guard</h3>
                <p className="text-sm text-white/50">This action cannot be undone</p>
              </div>
            </div>

            <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-4">
              <div className="font-semibold text-white">{deleteGuard.name}</div>
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                disabled={deleting}
                className="flex-1 rounded-xl border border-white/15 bg-white/5 py-3 text-sm font-bold text-white hover:bg-white/10 disabled:opacity-60"
              >
                Cancel
              </button>
              <button
      ...
      {showModal && created && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[#0d1f12] p-6 shadow-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-white">Guard Created!</h3>
                  <p className="text-sm text-white/50">Save these credentials now</p>
                </div>
              </div>
              <button
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="text-sm font-bold text-white">{created.name}</div>
                <div className="mt-1 text-sm text-white/50">{created.identifier}</div>

                <div className="mt-4 grid gap-3">
                  <div>
                    <div className="text-xs font-bold uppercase text-white/40">Password</div>
                    <div className="mt-1 flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/5 px-3 py-2">
                      <span className="font-mono text-sm font-bold text-white">
                        {created.password}
                      </span>
                      <button
                        type="button"
                        className="rounded-lg px-3 py-1.5 text-sm font-semibold text-white/50 hover:bg-white/10"
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
          ...
          <form className="mt-6 grid gap-4 md:grid-cols-2" onSubmit={onCreate}>
            ...
            {error && (
              <div className="md:col-span-2 rounded-xl border border-rose-500/20 bg-rose-500/10 px-4 py-3 text-sm font-semibold text-rose-300">
                {error}
              </div>
            )}
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-white">Registered Guards</h2>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="py-3 pr-4 text-xs font-bold uppercase tracking-wider text-white/40">
                      Name
                    </th>
                    <th className="py-3 pr-4 text-xs font-bold uppercase tracking-wider text-white/40">
                      Contact
                    ...
                    <th className="py-3 text-xs font-bold uppercase tracking-wider text-white/40">
                      Actions
                    </th>
                  </tr>
                </thead>
                    ...
                    <tr key={guard.userId} className="border-b border-white/5">
                      ...
                      <td className="py-3">
                        <button
                          ...
                          className="rounded-lg p-2 text-white/30 hover:bg-rose-500/15 hover:text-rose-400"
                          title="Delete guard"
                        >
