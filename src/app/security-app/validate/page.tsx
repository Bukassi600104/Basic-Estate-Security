...
export default function SecurityValidatePage() {
  ...
  async function validate() {
    ...
    try {
      ...
      const residentName = data.residentName ?? lookup?.code.resident.name ?? "Unknown";
      const houseNumber = data.houseNumber ?? lookup?.code.resident.houseNumber ?? "—";
      const generatedByName = data.generatedByName ?? lookup?.code.generatedBy?.name ?? residentName;
                ...
                {result.residentName && (
                  <div className="mt-4 rounded-xl bg-white/20 p-4">
                    ...
                    <p className="mt-1 text-lg opacity-90">
                      Generated by: {result.generatedByName ?? result.residentName}
                    </p>
          ...
          {lookup && (
            <div className="overflow-hidden rounded-2xl border-2 border-emerald-600/30 bg-white/5 shadow-lg animate-in slide-in-from-bottom-4">
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="rounded-xl bg-emerald-600/10 px-4 py-3">
                  <p className="text-xs text-white/50">Generated by</p>
                  <p className="mt-1 text-base font-bold text-white">
                    {lookup.code.generatedBy?.name ?? lookup.code.resident.name}
                  </p>
                </div>
                    ...
                    <div key={idx} className="flex items-center justify-between px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {v.generatedByName && (
                            <p className="text-xs text-white/40">By {v.generatedByName}</p>
                          )}
                        </div>
                      </div>
