...
import { listValidationLogsForEstatePage, type DdbCursor } from "@/lib/repos/validation-logs";
...
export async function GET(req: Request, { params }: { params: { estateId: string } }) {
  ...
  const page = await listValidationLogsForEstatePage({ estateId: params.estateId, limit, cursor });
